...
pub async fn pipeline(
  ...
  // 2. 取出所有 pipe 句柄
  let mut stdout_pipes: Vec<_> =
    children.iter_mut().map(|c| c.stdout.take()).collect();
  let mut stdin_pipes: Vec<_> =
    children.iter_mut().map(|c| c.stdin.take()).collect();
  ...
  let result = tokio::time::timeout(COMMAND_TIMEOUT, async {
    ...
    while let Some(res) = wait_set.join_next().await {
      let (i, status) =
        res.map_err(|e| anyhow!("Wait task panicked: {}", e))?;
      statuses[i] = Some(
        status
